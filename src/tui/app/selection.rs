...
impl App {
    ...
    pub fn clamp_inbox_selection(&mut self) {
        let len = self.inbox_visible_len();
        clamp_position(&mut self.inbox_selected, &mut self.inbox_list_offset, len);
    }

    pub fn clamp_drafts_selection(&mut self) {
        let len = self.drafts_visible_len();
        clamp_position(&mut self.drafts_selected, &mut self.drafts_list_offset, len);
    }

    pub fn clamp_sent_selection(&mut self) {
        let len = self.sent_visible_len();
        clamp_position(&mut self.sent_selected, &mut self.sent_list_offset, len);
    }
